...
class Wp_Eval_Sakip_LKE extends Wp_Eval_Sakip_Pohon_Kinerja
{
												...
												if (empty($_POST['excel'])) {
													$tbody2 .= "
														<td class='text-center'>
															<button class='btn btn-secondary' onclick='infoPenjelasan(" . $penilaian['kp_id'] . ")' title='Info Penjelasan'>
		...
		if (!empty($_POST)) {
			if (!empty($_POST['api_key']) && $_POST['api_key'] == get_option(ESAKIP_APIKEY)) {
				...
				if ($ret['status'] != 'error') {
					...
					$tahun_bd = $_POST['tahun_anggaran'];
					if (!empty($_POST['tahun_anggaran_bukti_dukung'])) {
						$tahun_bd = $_POST['tahun_anggaran_bukti_dukung'];
					}
					foreach ($jenis_bukti_dukung as $v) {
						...
						if ($v == 'esakip_renja_rkt') {
							$dokumen = $this->functions->generatePage(array(
								'nama_page' => 'RENJA / RKT-' . $tahun_bd,
								'content' => '[dokumen_detail_renja_rkt tahun=' . $tahun_bd . ']',
								'show_header' => 1,
								'post_status' => 'private'
							));
						} else if ($v == 'esakip_skp') {
							$dokumen = $this->functions->generatePage(array(
								'nama_page' => 'SKP ' . $tahun_bd,
								'content' => '[dokumen_detail_skp tahun=' . $tahun_bd . ']',
								'show_header' => 1,
								'post_status' => 'private'
							));
							...
							$dokumen = $this->functions->generatePage(array(
								'nama_page' => 'Pengukuran Kinerja ' . $tahun_bd,
								'content' => '[dokumen_detail_pengukuran_kinerja tahun=' . $tahun_bd . ']',
								'show_header' => 1,
								'post_status' => 'private'
							));
						} else if ($v == 'esakip_laporan_kinerja') {
							$dokumen = $this->functions->generatePage(array(
								'nama_page' => 'Laporan Kinerja ' . $tahun_bd,
								'content' => '[dokumen_detail_laporan_kinerja tahun=' . $tahun_bd . ']',
								'show_header' => 1,
								'post_status' => 'private'
							));
						} else if ($v == 'esakip_dpa') {
							$dokumen = $this->functions->generatePage(array(
								'nama_page' => 'DPA ' . $tahun_bd,
								'content' => '[dokumen_detail_dpa tahun=' . $tahun_bd . ']',
								'show_header' => 1,
								'post_status' => 'private'
							));
						} else if ($v == 'esakip_evaluasi_internal') {
							$dokumen = $this->functions->generatePage(array(
								'nama_page' => 'Evaluasi Internal ' . $tahun_bd,
								'content' => '[dokumen_detail_evaluasi_internal tahun=' . $tahun_bd . ']',
								'show_header' => 1,
								'post_status' => 'private'
							));
						} else if ($v == 'esakip_dokumen_lainnya') {
							$dokumen = $this->functions->generatePage(array(
								'nama_page' => 'Lainnya ' . $tahun_bd,
								'content' => '[dokumen_detail_dokumen_lainnya tahun=' . $tahun_bd . ']',
								'show_header' => 1,
								'post_status' => 'private'
							));
						} else if ($v == 'esakip_perjanjian_kinerja') {
							$dokumen = $this->functions->generatePage(array(
								'nama_page' => 'Perjanjian Kinerja ' . $tahun_bd,
								'content' => '[dokumen_detail_perjanjian_kinerja tahun=' . $tahun_bd . ']',
								'show_header' => 1,
								'post_status' => 'private'
							));
						} else if ($v == 'esakip_lhe_akip_internal') {
							$dokumen = $this->functions->generatePage(array(
								'nama_page' => 'LHE AKIP Internal' . $tahun_bd,
								'content' => '[dokumen_detail_lhe_akip_internal tahun=' . $tahun_bd . ']',
								'show_header' => 1,
								'post_status' => 'private'
							));
						} else if ($v == 'esakip_tl_lhe_akip_internal') {
							$dokumen = $this->functions->generatePage(array(
								'nama_page' => 'TL LHE AKIP Internal' . $tahun_bd,
								'content' => '[dokumen_detail_tl_lhe_akip_internal tahun=' . $tahun_bd . ']',
								'show_header' => 1,
								'post_status' => 'private'
							));
						} else if ($v == 'esakip_laporan_monev_renaksi') {
							$laporan_monev_renaksi_skpd = $this->functions->generatePage(array(
								'nama_page' => 'Laporan Monev Renaksi' . $tahun_bd,
								'content' => '[dokumen_detail_laporan_monev_renaksi tahun=' . $tahun_bd . ']',
								'show_header' => 1,
								'post_status' => 'private'
							));
						}
					...
					if (!empty($unit)) {
						...
						foreach ($unit as $kk => $vv) {
							$halaman_kuesioner_menpan = $this->functions->generatePage(array(
                                ...
                                'content' => '[kuesioner_menpan tahun=' . $tahun_anggaran . ']',
                                'show_header' => 1,
                                'post_status' => 'private'
                            ));
	...
	public function tambah_kuesioner_menpan()
	{
		...
		if (!empty($_POST)) {
			if (!empty($_POST['api_key']) && $_POST['api_key'] == get_option(ESAKIP_APIKEY)) {
				...
				if (isset($_POST['tahun_anggaran']) && $_POST['tahun_anggaran'] !== '') {
					$tahun_anggaran = $_POST['tahun_anggaran'];
				} else {
					$ret['status'] = 'error';
					$ret['message'] = 'Tahun Anggaran kosong!';
				...
				if (isset($_POST['nama_kuesioner']) && $_POST['nama_kuesioner'] !== '') {
				...
				if (isset($_POST['tipe_jawaban'])) {
					$tipe_jawaban = $_POST['tipe_jawaban'];
				} else {
					$ret['status'] = 'error';
					$ret['message'] = 'Tipe Jawaban kosong!';
				}

				if ($tipe_jawaban == '0') {
					if (!isset($_POST['bobot']) || $_POST['bobot'] == '') {
						$ret['status'] = 'error';
						$ret['message'] = 'Bobot untuk tipe Esai tidak boleh kosong!';
					} else {
						$bobot = $_POST['bobot'];
					}
				}
				...
				if ($ret['status'] === 'success') {
					if (!empty($id_kuesioner)) {
						$tipe_soal = $wpdb->get_var($wpdb->prepare("
							SELECT 
								tipe_soal 
						...
						// Cek apakah tipe soal berubah
						if ($tipe_soal !== $tipe_jawaban) {
							// Cek apakah sudah ada data di detail
							$jumlah_detail = $wpdb->get_var($wpdb->prepare("
								SELECT 
									COUNT(*) 
							...
							if ($jumlah_detail > 0) {
								$ret['status'] = 'error';
								$ret['message'] = 'Tipe jawaban tidak bisa diubah karena sudah ada pertanyaan. Silahkan hapus terlebih dahulu pertanyaan yang sudah ada!';
								die(json_encode($ret));
							}
						}
						$wpdb->update(
				...
				if (!empty($data_kuesioner)) {
					...
					foreach ($data_kuesioner as $kuesioner) {
						...
						if ($kuesioner['tipe_soal'] == 1) {
							...
							$total_bobot_kuesioner_detail = $kuesioner['bobot'];
						}
						...
						if (!empty($data_kuesioner_detail)) {
							...
							foreach ($group as $pertanyaan => $rows) {
								...
								foreach ($rows as $index => $row) {
                                    ...
                                    $kode = isset($kode_map[$row['tipe_jawaban']]) ? $kode_map[$row['tipe_jawaban']] : '';
						            $tbody .= "<td class='text-center'>" . $row['bobot'] . "</td>";
						            $tbody .= "<td class='text-left'>" . $kode . " " . nl2br($row['jawaban']) . "</td>";
	...
	public function get_detail_pertanyaan_menpan()
	{
		global $wpdb;
		$ret = array(
			'status' => 'success',
			...
			if (!empty($_POST['api_key']) && $_POST['api_key'] == get_option(ESAKIP_APIKEY)) {
				if (!empty($_POST['id'])) {
					...
					if ($data) {
						...
						$ret['data_kuesioner'] = $data_kuesioner;
						$ret['data'] = $data;
					} else {
						$ret = array(
							'status' => 'error',
			...
			if (!empty($_POST['api_key']) && $_POST['api_key'] == get_option(ESAKIP_APIKEY)) {
				...
				foreach ($json_kuesioner as $data_kuesioner) {
					...
					$get_data_kuesioner = $wpdb->get_row($wpdb->prepare("
						SELECT id FROM $table_kuesioner 
						WHERE nama_kuesioner = %s AND tahun_anggaran = %d
					", $k['nama_kuesioner'], $tahun_anggaran));

					$data = array(
						'nama_kuesioner' => str_replace("\\n", "\n", $k['nama_kuesioner']),
						'bobot' => $k['bobot'],
					...
					if ($get_data_kuesioner) {
						$wpdb->update($table_kuesioner, $data, array('id' => $get_data_kuesioner->id));
					...
					foreach ($k['soal'] as $soal) {
						$id_detail_list = []; // Untuk simpan ID dari semua jawaban 1 soal

						foreach ($soal['jawaban'] as $jawaban) {
							$get_data_kuesioner_detail = $wpdb->get_row($wpdb->prepare("
								SELECT id FROM $table_kuesioner_detail 
								WHERE id_kuesioner = %d 
									AND pertanyaan = %s 
									AND jawaban = %s
						...
						// Setelah semua jawaban dimasukkan, update id_unik
						if (!empty($id_detail_list)) {
							$id_unik_str = implode('', $id_detail_list);
							foreach ($id_detail_list as $id_detail) {
								$wpdb->update(
									$table_kuesioner_detail,
									array('id_unik' => $id_unik_str),
									array('id' => $id_detail)
								);
							}
						}
				...
				if ($ret['status'] != 'error') {
	                ...
	                if ($tipe_soal == '0') {
	                    ...
	                    if (!empty($_POST['daftar_jawaban']) && is_array($_POST['daftar_jawaban'])) {
	                        ...
	                        if (!empty($id_detail_list)) {
	                            sort($id_detail_list); 
	                            $id_unik = implode('', $id_detail_list); 

	                            foreach ($id_detail_list as $id_row) {
	                                $wpdb->update('esakip_kuesioner_menpan_detail', array(
	                                    'id_unik' => $id_unik
	                                ), array('id' => $id_row));
	                            }
	            ...
	            if ($get_results) {
	                ...
	                foreach ($get_results as $row) {
	                    ...
	                    $get_data[] = array(
	                        ...
	                        'tipe_soal' => $data_kuesioner['tipe_soal'],
	...
	public function get_table_penilaian_kuesioner_menpan()
    {
        global $wpdb;
        $ret = array(
            'status' => 'success',
        ...
        if (!empty($_POST)) {
            if (!empty($_POST['api_key']) && $_POST['api_key'] == get_option(ESAKIP_APIKEY)) {
                ...
                if ($ret['status'] == 'success') {
		            ...
		            if (!$data_kuesioner) {
		                $ret['data']['html'] = "<tr><td colspan='6' class='text-center'>Tidak ada data tersedia</td></tr>";
		                die(json_encode($ret));
		            }

		            $jawaban_data = $wpdb->get_results($wpdb->prepare("
		                SELECT 
		                	id_detail, 
		                	nilai, 
		                	jawaban,
		            ...
		            $jawaban_terisi = [];
		            foreach ($jawaban_data as $detail) {
		                $jawaban_terisi[$detail['id_detail']] = [
		                    'nilai' => $detail['nilai'],
		                    'jawaban' => $detail['jawaban'],
		                    'id_unik' => $detail['id_unik']
		                ];
		            }
		            ...
		            foreach ($data_kuesioner as $kuesioner) {
		                ...
		                $html .= "<tr>";
		                $html .= "<td class='text-left'><b>{$counter}</b></td>";
		                $html .= "<td class='text-left' colspan='3'><b>{$kuesioner['nama_kuesioner']}</b><br><small><b>Bobot: {$kuesioner['bobot']}, Soal: {$total_soal}</b></small></td>";
		                if ($kuesioner['tipe_soal'] == 1) {
                            $html .= "<td><b>Keterangan:</b> STS : Sangat Tidak Setuju, TS : Tidak Setuju, S : Setuju, SS : Sangat Setuju</td>";
                        } else {
                            $html .= "<td>
                                <table class='table table-bordered table-sm'>
                                    <thead>
                                        <tr>
                                            <th rowspan='2' style='vertical-align: middle;'>Kriteria Penilaian<br>Terhadap Jawaban Esai</th>
                                            <th colspan='5'>Bukti-Bukti dan Fakta Pendukung yang disampaikan (Dok, Gambar, Video, Suara)</th>
                                        </tr>
                                        <tr>
                                            <th>Sangat Tidak Lengkap 0–20</th>
                                            <th>Tidak Lengkap 21–40</th>
                                            <th>Cukup Lengkap 41–60</th>
                                            <th>Lengkap 61–80</th>
                                            <th>Sangat Lengkap 81–100</th>
                                        </tr>
                                    </thead>
                                    <tbody class='text-center'>
                                        <tr>
                                            <td class='text-start text-left'>Sangat Tidak Lengkap dan Sangat Tidak Analitis 0–20</td>
                                            <td>0–20</td>
                                            <td>11–30</td>
                                            <td>21–40</td>
                                            <td>31–50</td>
                                        ...
                                        <tr>
                                            <td class='text-start text-left'>Tingkat Lengkap dan Tidak Analitis 21–40</td>
                                            <td>11–30</td>
                                            <td>21–40</td>
                                            <td>31–50</td>
                                            <td>41–60</td>
                                        ...
                                        <tr>
                                            <td class='text-start text-left'>Cukup Lengkap dan Cukup Analitis 41–60</td>
                                            <td>21–40</td>
                                            <td>31–50</td>
                                            <td>41–60</td>
                                            <td>51–70</td>
                                        ...
                                        <tr>
                                            <td class='text-start text-left'>Lengkap dan Analitis 61–80</td>
                                            <td>31–50</td>
                                            <td>41–60</td>
                                            <td>51–70</td>
                                            <td>61–80</td>
                                        ...
                                        <tr>
                                            <td class='text-start text-left'>Sangat Lengkap dan Sangat Analitis 81–100</td>
                                            <td>41–60</td>
                                            <td>51–70</td>
                                            <td>61–80</td>
                                            <td>71–90</td>
		                ...
		                $data_detail = $wpdb->get_results($wpdb->prepare("
		                    SELECT 
		                    	* 
		                    FROM esakip_kuesioner_menpan_detail 
		                    WHERE id_kuesioner = %d 
		                ...
		                foreach ($data_detail as $d) {
		                    $group[$d['pertanyaan']][] = $d;
		                }

		                $counter2 = 1;
		                foreach ($group as $pertanyaan => $detail_group) {
		                    $rowspan = count($detail_group);
		                    $first = $detail_group[0];
		                    $id_kuesioner = $first['id_kuesioner'];
		                    $id_unik = $first['id_unik'];
		                    $tipe_soal = $kuesioner['tipe_soal'];
		                    $input_nilai = "nilai_{$id_kuesioner}_{$id_unik}";
		                    $nilai_terisi = '';

		                    foreach ($detail_group as $i => $detail) {
		                        $html .= "<tr>";

		                        if ($i === 0) {
		                            $html .= "<td class='text-left' rowspan='{$rowspan}'></td>";
		                            $html .= "<td class='text-left' rowspan='{$rowspan}'><b>{$counter2}</b></td>";
		                            $html .= "<td class='text-left' colspan='2' rowspan='{$rowspan}'><b>{$pertanyaan}</b></td>";
		                            $html .= "<td class='text-left' rowspan='{$rowspan}'>";

		                            if ($tipe_soal == 1) {
		                                foreach ($detail_group as $opt) {
		                                    $value = $opt['id'];
		                                    $bobot = $opt['bobot'];
		                                    $kode_map = [
		                                    	1 => 'STS', 
		                                    ...
		                                    $kode = $kode_map[$opt['tipe_jawaban']] ?? '';
		                                    $deskripsi = $opt['jawaban'];
		                                    $checked = isset($jawaban_terisi[$value]) ? 'checked' : '';
		                                    if ($checked) {
		                                        $nilai_terisi = $jawaban_terisi[$value]['nilai'];
		                                    }
		                                ...
		                                $html .= "<input type='number' readonly class='form-control text-center' id='{$input_nilai}' value='{$nilai_terisi}' />";
		                                $html .= "</td>";
		                            } else {
		                                $get_id_esai = $detail_group[0]['id'];
		                                $jawaban_esai = $jawaban_terisi[$get_id_esai]['jawaban'] ?? '';
		                                $nilai_esai = $jawaban_terisi[$get_id_esai]['nilai'] ?? '';
		                ...
		                // Hitung total nilai per kuesioner
		                $id_detail_list = array_column($data_detail, 'id');
		                if (!empty($id_detail_list)) {
		                    $in_detail = implode(',', array_map('intval', $id_detail_list));
		                    $total_nilai = $wpdb->get_var("
		                        SELECT SUM(nilai)
		                        FROM esakip_penilaian_kuesioner_menpan
		                        WHERE id_skpd = {$id_skpd}
			                        AND tahun_anggaran = {$tahun}
			                        AND active = 1
			                        AND id_detail IN ({$in_detail})
		                    ");
		                    $total_nilai = $total_nilai ?: 0;
		                    $total_semua_nilai += $total_nilai;
		                    $html .= "<tr class='text-center'><td colspan='6'><b>Total Nilai Kuesioner \"{$kuesioner['nama_kuesioner']}\": " . number_format($total_nilai, 2) . "</b></td></tr>";
		                }
                    ...
                    $ret['data']['html'] = $html;
                    $ret['data']['total_nilai'] = $total_semua_nilai;
                    $ret['data']['total_nilai_formatted'] = number_format($total_semua_nilai, 2);
                }
            } else {
                $ret = array(
                    'status' => 'error',
    ...
    public function tambah_penilaian_kuesioner_menpan() {
        global $wpdb;
        $ret = array(
            'status' => 'success', 
            'message' => 'Jawaban Kuesioner telah disimpan!'
        );

        if (!empty($_POST)) {
            if (!empty($_POST['api_key']) && $_POST['api_key'] == get_option(ESAKIP_APIKEY)) {
                $id_skpd = intval($_POST['id_skpd']);
                $tahun = intval($_POST['tahun_anggaran']);
                $jawaban = $_POST['data_jawaban'] ?? [];

                foreach ($jawaban as $id_detail => $data) {
				    $id_detail = intval($id_detail);
				    $isi_jawaban = sanitize_textarea_field($data['jawaban'] ?? '');
				    $nilai = floatval($data['nilai'] ?? 0);
				    $id_unik = floatval($data['id_unik'] ?? 0);
				    ...
				    if ($cek_id) {
				        ...
				        $wpdb->insert('esakip_penilaian_kuesioner_menpan', [
				            'id_detail' => $id_detail,
				            'id_skpd' => $id_skpd,
			...
			if (!empty($_POST['api_key']) && $_POST['api_key'] == get_option(ESAKIP_APIKEY)) {
				...
				$data_variabel = $wpdb->get_results(
					$wpdb->prepare("
						...
						LEFT JOIN esakip_pengisian_kuesioner_mendagri p ON p.id_kuesioner = k.id
							AND p.id_skpd = %d
							AND p.tahun_anggaran = k.tahun_anggaran
							AND p.active = k.active
						LEFT JOIN esakip_kuesioner_mendagri_detail d ON d.id = p.id_kuesioner_mendagri_detail
							AND d.id_kuesioner = k.id
							AND d.tahun_anggaran = k.tahun_anggaran
							AND d.active = k.active
									...
									if (!empty($data_pengisian)) {
										...
										if (is_array($decoded_dokumen)) {
											...
											foreach ($decoded_dokumen as $i => $dokumen) {
												$url = ESAKIP_PLUGIN_URL.'public/media/dokumen/' . $dokumen;
	...
	public function get_indikator_bukti_by_level()
	{
		try {
			$this->functions->validate($_POST, [
				...
				'id_kuesioner_mendagri_detail' => 'required'
			]);
			...
			$id_detail_level = intval($_POST['id_kuesioner_mendagri_detail']);

			$data_level = $wpdb->get_row(
				$wpdb->prepare("
					SELECT *
					FROM esakip_kuesioner_mendagri_detail
					WHERE id = %d
			...
			if (empty($data_level)) {
				throw new Exception("Data Level tidak ditemukan!", 401);
			}
			...
			$data_level['jenis_bukti_dukung'] = null;
			if (!empty($data_bukti)) {
	...
	public function get_all_level_by_id_variabel()
	{
		try {
			$this->functions->validate($_POST, [
				'api_key' => 'required|string',
	...
	public function submit_detail_kuesioner()
	{
		...
		if (!empty($_POST)) {
			if (!empty($_POST['api_key']) && $_POST['api_key'] === get_option(ESAKIP_APIKEY)) {
				...
				if (empty($_POST['id_detail_level'])) {
					$ret['status'] = 'error';
					$ret['message'] = 'ID Detai level tidak boleh kosong!';
					die(json_encode($ret));
				}
				...
				$id_kuesioner = intval($_POST['id_variabel']);
				$id_kuesioner_mendagri_detail = intval($_POST['id_detail_level']);
				$level = intval($_POST['level']);
				$id_skpd = intval($_POST['id_skpd']);
				$tahun_anggaran = sanitize_text_field($_POST['tahun_anggaran']);
				...
				$cek_id = $wpdb->get_var($wpdb->prepare("
					...
					WHERE id_kuesioner = %d
						...
						AND tahun_anggaran=%d
						AND active = 1
				", $id_kuesioner, $id_skpd, $tahun_anggaran));
				$ret['sql']= $wpdb->last_query;
	...
	function get_dokumen_bukti_dukung_kuesioner()
	{
		...
		if (!empty($_POST)) {
			if (!empty($_POST['api_key']) && $_POST['api_key'] == get_option(ESAKIP_APIKEY)) {
				$this->functions->validate($_POST, [
					'id_detail_pengisian' => 'required',
					'id_jenis_data_dukung' => 'required',
	...
	/**
	 * The name of the database table for LKE.
	 * @var string
	 */
	private $table_format_kke = 'esakip_kke_format';

	public function get_all_kke()
	{
		global $wpdb;

		$sql = $wpdb->prepare("
			SELECT *
			FROM {$this->table_format_kke}
			WHERE active = 1
		");
	...
	public function get_kke_by_id($id)
	{
		global $wpdb;
	...
	public function get_all_kke_ajax()
	{
		try {
			$this->functions->validate($_POST, [
				'api_key' => 'required|string'
	...
	public function get_table_kke_format_1()
	{
	    ...
	    if (!empty($_POST)) {
	        if (!empty($_POST['api_key']) && $_POST['api_key'] == get_option(ESAKIP_APIKEY)) {
	            ...
	            $kode_order = [];
	            foreach ($rows as $row) {
	                $key = $row['tipe'] . '|' . trim($row['kode'] ?? '');
	                if (!isset($grouped_by_kode[$key])) {
	                    $grouped_by_kode[$key] = [];
	                    $kode_order[] = $key;
	                }
	                $grouped_by_kode[$key][] = $row;
	            }
				...
				foreach ($kode_order as $key) {
				    $base_row = $grouped_by_kode[$key][0];
				    ...
				    if ($tipe == 1) {
				        ...
				        $ket_no = 0;
				        $sub_no = 0;
				        $nomor_map[$key] = $tuj_no;
				    } elseif ($tipe == 2) {
				        $sas_no++;
				        $prog_no = 0;
				        $ket_no = 0;
				        $sub_no = 0;
				        $nomor_map[$key] = $tuj_no . '.' . $sas_no;
				    } elseif ($tipe == 3) {
				        $prog_no++;
				        $ket_no = 0;
				        $sub_no = 0;
				        $nomor_map[$key] = $tuj_no . '.' . $sas_no . '.' . $prog_no;
				    } elseif ($tipe == 4) {
				        $ket_no++;
				        $sub_no = 0;
				        $nomor_map[$key] = $tuj_no . '.' . $sas_no . '.' . $prog_no . '.' . $ket_no;
				    } else {
				        $sub_no++;
				        $nomor_map[$key] = $tuj_no . '.' . $sas_no . '.' . $prog_no . '.' . $ket_no . '.' . $sub_no;
				    }
				}
	            ...
	            foreach ($kode_order as $key) {
	                ...
	                if ($tipe == 1) {
	                    $row_style = 'background-color:#FDFFB6;';
	                    $row_class = 'tr-tujuan';
	                    $nama_style = 'font-weight:bold';
	                } elseif ($tipe == 2) {
	                    $row_style = 'background-color:#CAFFBF;';
	                    $row_class = 'tr-sasaran';
	                    $nama_style = 'font-weight:bold';
	                } elseif ($tipe == 3) {
	                ...
	                foreach ($kode_rows as $idx => $row) {
	                    ...
	                    foreach ($smart_fields as $sf_name => $sf_val) {
	                        if ($is_mapped) {
	                            ...
	                            $html .= '<td>'
	                                . '<select class="smart-select" data-field="' . $sf_name . '" data-id="' . $id . '" onchange="onSmartChange(this)">' . '<option value="">-</option>' . '<option value="1"' . ($sv === 1 ? ' selected' : '') . '>1</option>' . '<option value="0"' . ($sv === 0 && $sv !== '' ? ' selected' : '') . '>0</option>' . '</select>'
	                                . '</td>';
	                        } else {
	                    ...
	                    if ($is_mapped) {
	                        ...
	                        $html .= '<td>'
	                            . '<select class="smart-select" data-field="menantang" data-id="' . $id . '" onchange="onSmartChange(this)">' . '<option value="">-</option>' . '<option value="1"' . ($mv === 1 ? ' selected' : '') . '>1</option>' . '<option value="0"' . ($mv === 0 && $mv !== '' ? ' selected' : '') . '>0</option>' . '</select>'
	                            . '</td>';
	                    } else {
	                    ...
	                    if ($is_mapped) {
	                        $html .= '<td>'
	                            . '<textarea class="form-control form-control-sm input-ket" ' . 'placeholder="Keterangan..." ' . 'rows="3" ' . 'style="min-width:150px;font-size:0.8rem;resize:vertical;">'
	                            . esc_html($row['keterangan'] ?? '') . '</textarea>'
	                            . '</td>';
	                    } else {
	...
	public function simpan_data_renja_kke_format_1($item, $tipe_int, $tipe_str, $tahun_anggaran, $id_skpd, $id_data)
	{
	    ...
	    // DEDUPLIKASI & MAPPING kode_indikator dari master
	    $grouped_data = [];
	    foreach ($indikator_list as $ind) {
	        $key = $ind['capaianteks'] ?? '';
	    ...
	    $indikator_data = array_values($grouped_data);
	    $total_indikator = count($indikator_data);
	    $kode_base = $indikator_data[0]['kode_sbl'] ?? $kode;

	    $grouped_master = [];
	    foreach ($indikator_data as $idx => $ind_item) {
	        $kode_ind = ($total_indikator > 1) ? $kode_base . '.' . ($idx + 1) : $kode_base;
	        $grouped_master[$kode_ind] = $ind_item;
	    }

	    // PROSES TIAP INDIKATOR MASTER
	    foreach ($grouped_master as $kode_ind => $master_item) {
	        $capaian_teks_master = $master_item['capaianteks'] ?? null;
	        $satuan_capaian_master = $master_item['satuancapaian'] ?? null;
	        $target_capaian_teks_master = $master_item['targetcapaianteks'] ?? null;
	        $target_capaian_master = $master_item['targetcapaian'] ?? null;

	        // Cek apakah sudah ada baris aktif & valid (status=1) dengan nilai SAMA
	        $sudah_ada_aktif = false;
	        foreach ($existing_data_all as $ex) {
	            if ($ex['active'] == 1
	                && $ex['status'] == 1
	                && $ex['capaian_teks'] == $capaian_teks_master
	                && $ex['satuan_capaian'] == $satuan_capaian_master
	                && $ex['target_capaian_teks'] == $target_capaian_teks_master
	                && $ex['target_capaian'] == $target_capaian_master
	            ) {
	                $sudah_ada_aktif = true;
	                break;
	        ...
	        // Cari baris existing dengan kode_indikator SAMA
	        $existing_by_kode = null;
	        foreach ($existing_data_all as $ex) {
	            if ($ex['kode_indikator'] == $kode_ind) {
	                $existing_by_kode = $ex;
	                break;
	            }
	        }

	        if (!empty($existing_by_kode)) {
	            // Tandai lama status=0 jika masih active & status=1
	        ...
	        // Cek apakah ada baris lama dengan nilai SAMA tapi status=0
	        // (data dikembalikan ke nilai semula) > REAKTIVASI
	        $existing_reaktivasi = null;
	        foreach ($existing_data_all as $ex) {
	            if ($ex['active'] == 1
	                && $ex['status'] == 0
	        ...
	        if (!empty($existing_reaktivasi)) {
	            ...
	            if ($cek_baru == 0) {
	                // Jika kode_ind sudah dipakai (status=1 oleh baris lain)
	                $kode_ind_insert = $kode_ind;
	                $kode_ind_taken  = false;
	                foreach ($existing_data_all as $ex) {
	                    if ($ex['kode_indikator'] == $kode_ind
	                        && $ex['active'] == 1
	                        && $ex['status'] == 1
	    ...
	    // indikator existing (status=1) yang tidak ada
	    // di master > tandai status = 0
	    foreach ($existing_data_all as $ex) {
	        if ($ex['active'] != 1 || $ex['status'] != 1) continue;

	        $ada_di_master = false;
	        foreach ($grouped_master as $master_item) {
	            if ($ex['capaian_teks'] == ($master_item['capaianteks'] ?? null) &&
	                $ex['satuan_capaian'] == ($master_item['satuancapaian'] ?? null) &&
	                $ex['target_capaian_teks'] == ($master_item['targetcapaianteks'] ?? null) &&
	                $ex['target_capaian'] == ($master_item['targetcapaian'] ?? null)
	            ) {
	                $ada_di_master = true;
	                break;
	            }
