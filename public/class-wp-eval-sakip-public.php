...
class Wp_Eval_Sakip_Public extends Wp_Eval_Sakip_Verify_Dokumen
{
							...
							if ($edit_pa) {
								$wpdb->update(
									...
									array(
										...
										'nama_tabel_dokumen' => 'esakip_renja_rkt'
									),
									array('%d'),
									array('%d', '%s')
								);
						...
						if ($wpdb->rows_affected == 0) {
							...
							if ($edit_pa) {
								$wpdb->update(
									...
									array(
										'id_dokumen' => $id_dokumen,
										'nama_tabel_dokumen' => 'esakip_perjanjian_kinerja'
									),
									array('%d'),
									array('%d', '%s')
								);
						...
						if ($wpdb->rows_affected == 0) {
							...
							if ($edit_pa) {
								$wpdb->update(
									...
									array(
										'id_dokumen' => $id_dokumen,
										'nama_tabel_dokumen' => 'esakip_laporan_kinerja'
									),
									array('%d'),
									array('%d', '%s')
								);
		...
		if (!empty($_POST)) {
			if (!empty($_POST['api_key']) && $_POST['api_key'] == get_option(ESAKIP_APIKEY)) {
				...
				if (!empty($rencana_aksis)) {
					...
					if ($status_api_esr && !empty($mapping_jenis_dokumen_esr)) {
						foreach ($array_data_esr as $data_esr) {
							foreach ($rencana_aksis as $esr_lokal) {
								if (!empty($esr_lokal['upload_id']) && $data_esr['upload_id'] != $esr_lokal['upload_id']) {
									if (empty($non_esr_lokal[$data_esr['nama_file']])) {
										$non_esr_lokal[$data_esr['nama_file']] = $data_esr;
									}
		...
		if (!empty($_POST)) {
			if (!empty($_POST['api_key']) && $_POST['api_key'] == get_option(ESAKIP_APIKEY)) {
				...
				if (!empty($perjanjian_kinerjas)) {
					...
					if ($status_api_esr && !empty($mapping_jenis_dokumen_esr)) {
						foreach ($array_data_esr as $data_esr) {
							foreach ($perjanjian_kinerjas as $esr_lokal) {
								if (!empty($esr_lokal['upload_id']) && $data_esr['upload_id'] != $esr_lokal['upload_id']) {
									if (empty($non_esr_lokal[$data_esr['nama_file']])) {
										$non_esr_lokal[$data_esr['nama_file']] = $data_esr;
									}
		...
		if (!empty($_POST)) {
			if (!empty($_POST['api_key']) && $_POST['api_key'] == get_option(ESAKIP_APIKEY)) {
				...
				if (!empty($laporan_kinerjas)) {
					...
					if ($status_api_esr && !empty($mapping_jenis_dokumen_esr)) {
						foreach ($array_data_esr as $data_esr) {
							foreach ($laporan_kinerjas as $esr_lokal) {
								if (!empty($esr_lokal['upload_id']) && $data_esr['upload_id'] != $esr_lokal['upload_id']) {
									if (empty($non_esr_lokal[$data_esr['nama_file']])) {
										$non_esr_lokal[$data_esr['nama_file']] = $data_esr;
									}
		...
		if (!empty($_POST)) {
			if (!empty($_POST['api_key']) && $_POST['api_key'] == get_option(ESAKIP_APIKEY)) {
				...
				if (!empty($dokumen_lains)) {
					...
					if ($status_api_esr && !empty($mapping_jenis_dokumen_esr)) {
						foreach ($array_data_esr as $data_esr) {
							foreach ($dokumen_lains as $esr_lokal) {
								if (!empty($esr_lokal['upload_id']) && $data_esr['upload_id'] != $esr_lokal['upload_id']) {
									if (empty($non_esr_lokal[$data_esr['nama_file']])) {
										$non_esr_lokal[$data_esr['nama_file']] = $data_esr;
									}
	...
	public function get_table_rpjmd()
	{
		...
		$ret = array(
			'status' => 'success',
			'message' => 'Berhasil get data!',
			'tahun_anggaran_periode_dokumen' => null,
			'status_mapping_esr' => false,
			'data' => array()
		);
			...
			if (!empty($_POST['api_key']) && $_POST['api_key'] == get_option(ESAKIP_APIKEY)) {
				...
				if ($ret['status'] == 'success') {
					...
					$nama_tabel_admin = array(
						...
						"dokumen_lainnya" => "esakip_other_file",
											...
											array( //8
												...
												'data' => array(
													...
													array(
														...
														'pesan_kesalahan' => 'SALAH, Indikator Tujuan belum SMART.'
													)
												),
												'jenis_bukti_dukung' => array(
													'esakip_renstra',
		...
		if (!empty($_POST)) {
			if (!empty($_POST['api_key']) && $_POST['api_key'] == get_option(ESAKIP_APIKEY)) {
				...
				if (!empty($laporan_monev_renaksis)) {
					...
					if ($status_api_esr && !empty($mapping_jenis_dokumen_esr)) {
						foreach ($array_data_esr as $data_esr) {
							foreach ($laporan_monev_renaksis as $esr_lokal) {
								if (!empty($esr_lokal['upload_id']) && $data_esr['upload_id'] != $esr_lokal['upload_id']) {
									if (empty($non_esr_lokal[$data_esr['nama_file']])) {
										$non_esr_lokal[$data_esr['nama_file']] = $data_esr;
									}
		...
		if (!empty($_POST)) {
			if (!empty($_POST['api_key']) && $_POST['api_key'] == get_option(ESAKIP_APIKEY)) {
				...
				if (!empty($dokumen_unset)) {
					...
					if ($status_api_esr && !empty($mapping_jenis_dokumen_esr)) {
						foreach ($array_data_esr as $data_esr) {
							foreach ($dokumen_unset as $esr_lokal) {
								if (!empty($esr_lokal['upload_id']) && $data_esr['upload_id'] != $esr_lokal['upload_id']) {
									if (empty($non_esr_lokal[$data_esr['nama_file']])) {
										$non_esr_lokal[$data_esr['nama_file']] = $data_esr;
									}
	...
	function menu_eval_sakip()
	{
		...
		if (
			...
			//Laporan PK Page (per tahun) for ADMIN
			if (!empty($_GET['tahun'])) {
				$laporan_pk_page = $this->functions->generatePage(array(
					'nama_page' => 'Laporan PK Perangkat Daerah - ' . $_GET['tahun'],
					'content' => '[list_halaman_laporan_pk tahun=' . $_GET['tahun'] . ']',
					'show_header' => 1,
					'post_status' => 'private'
				));
				$laporan_pk_btn = '<li><a target="_blank" href="' . $laporan_pk_page['url'] . '" class="btn btn-primary">Laporan PK (Perjanjian Kinerja)</a></li>';
			}

			$halaman_laporan_pk_admin = '
			<div class="accordion">
				<h5 class="esakip-header-tahun" data-id="laporan-pk-' . $_GET['tahun'] . '" style="margin: 0;">Laporan PK (Perjanjian Kinerja)</h5>
				<div class="esakip-body-tahun" data-id="laporan-pk-' . $_GET['tahun'] . '">
					<ul style="margin-left: 20px; margin-bottom: 10px; margin-top: 5px;">
						' . $laporan_pk_btn . '
					</ul>
				</div>
			...
			$skpd_db_datas = array_merge($skpd_db_datas, $skpd_db_datas_by_simpeg);

			if(!empty($skpd_db_datas)){
				$proteksi_skpd_sama = array();
				foreach ($skpd_db_datas as $skpd_db) {
					// mencegah id skpd sama diulang dua kali
					if(in_array($skpd_db['id_skpd'], $proteksi_skpd_sama)){
						continue;
					}
					$proteksi_skpd_sama[] = $skpd_db['id_skpd'];
					///// -------------------------- /////
					...
					//Laporan PK Page (per tahun) for SKPD
					if (!empty($_GET['tahun'])) {
						$laporan_pk_page = $this->functions->generatePage(array(
		...
		if (
			...
			$nip_pegawai = get_user_meta($user_id, '_nip') ?: get_user_meta($user_id, 'nip');
			$tahun_skpd = get_option('_crb_tahun_wpsipd');
			$cek_menu_satu_skpd_satu_pegawai = array();
			$data_pegawai_simpeg = $wpdb->get_results(
				$wpdb->prepare(
					"SELECT
						dp.id, 
						dp.nip_baru, 
						...
						dp.active  
					FROM
						esakip_data_pegawai_simpeg dp
					WHERE
						dp.nip_baru=%s
						AND dp.active=%d
					ORDER BY dp.satker_id ASC, dp.tipe_pegawai_id ASC",
					$nip_pegawai[0],
					1
				),
				ARRAY_A
			...
			if (!empty($data_pegawai_simpeg)) {
				foreach ($data_pegawai_simpeg as $key => $data_pegawai) {
					...
					if(
						$data_pegawai['plt_plh'] == 1	
						&& (
							$data_pegawai['tmt_sk_plth'] > date('Y-m-d H:i:s')
							|| date('Y-m-d H:i:s') > $data_pegawai['berakhir']
						)
					){
						continue;
					}
					...
					$data_pegawai['nama_bidang'] = '';
					if(!empty($get_nama_bidang) && !empty($get_nama_bidang['nama'])){
						$data_pegawai['nama_bidang'] = $get_nama_bidang['nama'];
					}
					// end //

					$skpd_db_datas = $wpdb->get_results(
						$wpdb->prepare(
							"SELECT 
								simpeg.id_satker_simpeg,
								unit.nama_skpd, 
					...
					foreach ($skpd_db_datas as $skpd_db) {
						...
						if(in_array($skpd_db['id_skpd'], $cek_menu_satu_skpd_satu_pegawai)){
							continue;
						}else{
							$cek_menu_satu_skpd_satu_pegawai[] = $skpd_db['id_skpd'];
						}
						...
						if (!empty($pengaturan_periode_dokumen)) {
							...
							$mapping_jenis_dokumen_esr = $wpdb->get_row($wpdb->prepare("
				                        ...
				                        WHERE 
				                            a.tahun_anggaran=%d AND
				                            b.nama_tabel=%s;
							", $pengaturan_periode_dokumen['tahun_anggaran'], 'esakip_rpjpd'), ARRAY_A);
		...
		if (!empty($_POST)) {
			if (!empty($_POST['api_key']) && $_POST['api_key'] == get_option(ESAKIP_APIKEY)) {
				...
				if (!empty($data_pohon_kinerja)) {
					...
					if ($status_api_esr && !empty($mapping_jenis_dokumen_esr)) {
						foreach ($array_data_esr as $data_esr) {
							foreach ($data_pohon_kinerja as $esr_lokal) {
								if (!empty($esr_lokal['upload_id']) && $data_esr['upload_id'] != $esr_lokal['upload_id']) {
									if (empty($non_esr_lokal[$data_esr['nama_file']])) {
										$non_esr_lokal[$data_esr['nama_file']] = $data_esr;
									}
